...
class History( BaseUIController ):
    ...
    def history_and_dataset_per_user( self, trans, **kwd ):
        ...
        histories = sa.select( ( sa.func.count( galaxy.model.History.table.c.id ).label( 'history' ),
        ...
        # group by u.email;
        datasets = sa.select( ( sa.func.count( galaxy.model.Dataset.table.c.id ).label( 'dataset' ),
        ...
        datasets = dict( [ ( _.email if _.email is not None else "Unknown",
            (int( _.dataset ), int( _.size )) ) for _ in datasets.execute() ] )
    ...
    def history_and_dataset_type( self, trans, **kwd ):
        ...
        # execute requests, replace None fields by "Unknown"
        data = [ ( _.name if _.name is not None else "NoNamedHistory",
            _.state ) for _ in histories.execute() ]
